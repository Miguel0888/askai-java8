...
public final class ProxyConfiguration {
    ...
    public ProxyMode getMode() { return toWinProxyMode(mode); }
    ...
    private com.aresstack.winproxy.ProxyConfiguration toWinProxyConfiguration(ProxyMode configuredMode, String effectivePacUrl) {
        com.aresstack.winproxy.ProxyConfiguration.Builder builder = com.aresstack.winproxy.ProxyConfiguration.builder().mode(configuredMode).testUrl(testUrl).pacUrlDiscoveryScript(pacUrlDiscoveryScript);
        if (trimToNull(effectivePacUrl) != null) { builder.pacUrl(effectivePacUrl); }
        if (trimToNull(manualProxyHost) != null) { builder.manualProxyHost(manualProxyHost); }
        if (manualProxyPort > 0) { builder.manualProxyPort(manualProxyPort); }
        return builder.build();
    }

    public Object resolve(String url) {
        try { return resolveByMode(nonBlank(url, testUrl)); }
        catch (IOException ex) { return "ERROR (" + ex.getMessage() + ")"; }
    }

    public Proxy toProxy() { return toProxyFor(testUrl); }

    public Proxy toProxyFor(String url) {
        try { return resolveJavaProxy(url); }
        catch (IOException ex) { return Proxy.NO_PROXY; }
    }

    public Proxy resolveJavaProxy(String url) throws IOException {
        validateForUse();
        if (DISABLED.equals(mode)) { return Proxy.NO_PROXY; }
    ...
    public void validateForUse() throws IOException {
        ...
        if (MANUAL_PROXY.equals(mode)) {
            if (trimToNull(manualProxyHost) == null || manualProxyPort <= 0) { throw new IOException("MANUAL_PROXY requires manual host and port."); }
            return;
        }
        if (PAC_URL_MANUAL.equals(mode) && trimToNull(pacUrlDiscoveryScript) == null) { throw new IOException("PAC_URL_MANUAL requires the PAC/WPAD URL in the PAC URL discovery script field."); }
        if (PAC_URL_WSCRIPT.equals(mode) && trimToNull(pacUrlDiscoveryScript) == null) { throw new IOException("PAC_URL_WSCRIPT requires a script in the PAC URL discovery script field."); }
    }

    private String manualPacUrl() { return PAC_URL_MANUAL.equals(mode) ? pacUrlDiscoveryScript : pacUrl; }

    private ProxyMode toWinProxyMode(String configuredMode) {
        if (PAC_URL_WSCRIPT.equals(configuredMode)) { return ProxyMode.PAC_URL_MANUAL; }
    ...
    private static String normalizeMode(String value) {
        ...
        if (PAC_URL_WSCRIPT.equals(candidate)) { return candidate; }
        try { return ProxyMode.valueOf(candidate).name(); }
        catch (Exception ex) { return PAC_URL_POWERSHELL; }
    }

    private static String defaultDiscoveryScript(String configuredMode) {
        if (PAC_URL_WSCRIPT.equals(configuredMode)) { return PacUrlDiscoveryService.defaultScript(); }
    ...
    private Proxy reflectProxy(Object result) {
        ...
        for (int i = 0; i < proxyMethods.length; i++) {
            try {
                ...
                if (value instanceof Proxy) { return (Proxy) value; }
        ...
        if (host != null && host.length() > 0 && port != null && port.intValue() > 0) { return new Proxy(Proxy.Type.HTTP, new InetSocketAddress(host, port.intValue())); }
        return null;
    }
    ...
    private String stringMethod(Object target, String first, String second, String third, String fourth) {
        ...
        for (int i = 0; i < names.length; i++) {
            try { Object value = target.getClass().getMethod(names[i]).invoke(target); if (value != null) { return String.valueOf(value); } }
            catch (Exception ignored) {}
        }
        return null;
    }
    ...
    private Integer intMethod(Object target, String first, String second, String third, String fourth) {
        ...
        for (int i = 0; i < names.length; i++) {
            try {
                Object value = target.getClass().getMethod(names[i]).invoke(target);
                if (value instanceof Number) { return Integer.valueOf(((Number) value).intValue()); }
                if (value != null) { return Integer.valueOf(String.valueOf(value)); }
            } catch (Exception ignored) {}
        }
        return null;
    }
    ...
    private Proxy parseProxy(String value) {
        if (value == null || value.toUpperCase().indexOf("DIRECT") >= 0) { return Proxy.NO_PROXY; }
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (colon > 0 && colon + 1 < token.length()) {
                ...
                try { int port = Integer.parseInt(portText); if (host.length() > 0 && port > 0) { return new Proxy(Proxy.Type.HTTP, new InetSocketAddress(host, port)); } }
    ...
    private static String defaultPacScript(com.aresstack.winproxy.ProxyConfiguration cfg) { return nonBlank(cfg.getPacUrlDiscoveryScript(), ProxyDefaults.DEFAULT_PAC_URL_DISCOVERY_SCRIPT); }
    private static String nonBlank(String value, String fallback) { String trimmed = trimToNull(value); return trimmed == null ? fallback : trimmed; }
    private static String trimToNull(String value) { if (value == null) { return null; } String trimmed = value.trim(); return trimmed.length() == 0 ? null : trimmed; }

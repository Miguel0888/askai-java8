...
final class RenderedPageDocumentCapture {
    ...
    String captureScript() {
        ...
        return "() => {\n"
                ...
                + "    const parentText = a.parentElement ? clean(a.parentElement.textContent) : '';\n"
                + "    links.push({\n"
                + "      id: 'link-' + pad(++linkSeq), containerId: containerId,\n"
                + "      href: a.href || '', text: clean(a.textContent).slice(0, 300),\n"
                + "      surrounding: parentText.slice(0, EXCERPT),\n"
                ...
                + "    if (/^H[1-6]$/.test(tag)) { stats.headings++; ctx.lastHeading = clean(node.textContent); }\n"
                ...
                + "      ].filter(Boolean),\n"
                + "      textExcerpt: clean(el.textContent).slice(0, EXCERPT),\n"

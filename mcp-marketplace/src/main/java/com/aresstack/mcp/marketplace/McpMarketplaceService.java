...
public final class McpMarketplaceService {
    ...
    public McpMarketplaceService() {
        this(MarketplaceConfiguration.createDefault());
    }

    public McpMarketplaceService(MarketplaceConfiguration configuration) {
        if (configuration == null) {
            throw new IllegalArgumentException("configuration must not be null");
        }
        McpMarketplaceHttpClient httpClient = new McpMarketplaceHttpClient(configuration);
        this.sources = Collections.unmodifiableList(new ArrayList<>(configuration.getSources()));
    ...
    public List<McpMarketplaceEntry> search(String query, String requestedSourceId, boolean forceRefresh) {
        ...
        for (McpMarketplaceSource source : sources) {
            ...
            try {
                ...
                log.warn("Failed to load MCP marketplace source " + source.getName() + ": " + e.getMessage(), e);
            }
        }
